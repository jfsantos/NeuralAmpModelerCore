...
static constexpr size_t NUM_BENCHMARK_BUFFERS = (static_cast<size_t>(SAMPLE_RATE) / AUDIO_BUFFER_SIZE) * BENCHMARK_DURATION_SECONDS;
...
void RunBenchmark(std::unique_ptr<nam::DSP>& model, const char* filename)
{
    ...
    Printf("  Time: %lu.%02lu ms | Audio: %lu ms | RT: %lu.%02lux | %s",
           (unsigned long)totalTimeMs,
           (unsigned long)totalTimeFrac,
           (unsigned long)audioTimeMs,
           (unsigned long)(realTimePercent / 100),
           (unsigned long)(realTimePercent % 100),
           realTimePercent >= 100 ? "OK" : "SLOW");
}
...
int main(void)
{
    ...
    while (true)
    {
        PrintLine("[DEBUG] Calling f_readdir...");
        result = f_readdir(&dir, &fno);
        Printf("[DEBUG] f_readdir returned %d, fname='%s'", (int)result, fno.fname);

        if (result != FR_OK || fno.fname[0] == 0)
        {
            PrintLine("[DEBUG] End of directory or error");
        ...
        // Load the model
        PrintLine("[DEBUG] Loading model...");
        std::unique_ptr<nam::DSP> model = nam::get_dsp_fatfs(fno.fname);
        PrintLine("[DEBUG] Model load returned");
        ...
        // Run benchmark
        PrintLine("[DEBUG] Running benchmark...");
        RunBenchmark(model, fno.fname);
        PrintLine("[DEBUG] Benchmark complete");
        successCount++;

        // Release model memory before loading next
        PrintLine("[DEBUG] Releasing model...");
        model.reset();
        PrintLine("[DEBUG] Model released");
    }
