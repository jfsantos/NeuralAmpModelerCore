...
nam::activations::ActivationFastTanh _FAST_TANH =
    nam::activations::ActivationFastTanh();
nam::activations::ActivationHardTanh _HARD_TANH =
    nam::activations::ActivationHardTanh();
nam::activations::ActivationReLU _RELU = nam::activations::ActivationReLU();
nam::activations::ActivationLeakyReLU _LEAKY_RELU =
    nam::activations::ActivationLeakyReLU(
        0.01);  // FIXME does not parameterize LeakyReLU
nam::activations::ActivationPReLU _PRELU =
    nam::activations::ActivationPReLU(0.01);  // Same as leaky ReLU by default
nam::activations::ActivationSigmoid _SIGMOID =
    nam::activations::ActivationSigmoid();
nam::activations::ActivationSwish _SWISH = nam::activations::ActivationSwish();
nam::activations::ActivationHardSwish _HARD_SWISH =
    nam::activations::ActivationHardSwish();
nam::activations::ActivationLeakyHardTanh _LEAKY_HARD_TANH =
    nam::activations::ActivationLeakyHardTanh();

bool nam::activations::Activation::using_fast_tanh = false;

std::unordered_map<std::string, nam::activations::Activation*>
    nam::activations::Activation::_activations = {
        {"Tanh", &_TANH},
        {"Hardtanh", &_HARD_TANH},
        {"Fasttanh", &_FAST_TANH},
        {"ReLU", &_RELU},
        ...
        {"LeakyHardtanh", &_LEAKY_HARD_TANH},
        {"PReLU", &_PRELU}};

nam::activations::Activation* tanh_bak = nullptr;
nam::activations::Activation* sigmoid_bak = nullptr;

nam::activations::Activation* nam::activations::Activation::get_activation(
    const std::string name) {
  if (_activations.find(name) == _activations.end()) return nullptr;
...
void nam::activations::Activation::enable_lut(std::string function_name,
                                              float min, float max,
                                              std::size_t n_points) {
  ...
  if (function_name == "Tanh") {
    ...
    throw std::runtime_error(
        "Tried to enable LUT for a function other than Tanh or Sigmoid");
  }
...
void nam::activations::Activation::disable_lut(std::string function_name) {
  if (function_name == "Tanh") {
    ...
    throw std::runtime_error(
        "Tried to disable LUT for a function other than Tanh or Sigmoid");
  }
}
