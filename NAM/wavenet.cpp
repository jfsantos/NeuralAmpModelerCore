...
nam::wavenet::_DilatedConv::_DilatedConv(const int in_channels,
...
void nam::wavenet::_Layer::process_(const Eigen::MatrixXf& input,
  ...
  if (!this->_gated) {
    ...
    // CAREFUL: .topRows() and .bottomRows() won't be memory-contiguous for a
    // column-major matrix (Issue 125). Need to do this column-wise:
    for (int i = 0; i < num_frames; i++) {
      this->_activation->apply(this->_z.block(0, i, channels, 1));
      // TODO Need to support other activation functions here instead of
      // hardcoded sigmoid
      activations::Activation::get_activation("Sigmoid")->apply(
          this->_z.block(channels, i, channels, 1));
    }
    this->_z.block(0, 0, channels, num_frames).array() *=
        this->_z.block(channels, 0, channels, num_frames).array();
  }

  head_input.leftCols(num_frames).noalias() +=
      this->_z.block(0, 0, channels, num_frames);
  if (!_gated) {
  ...
  output.middleCols(j_start, ncols).noalias() =
      input.middleCols(i_start, ncols) + _1x1.GetOutput(num_frames);
}

void nam::wavenet::_Layer::set_num_frames_(const long num_frames) {
  // TODO deprecate for SetMaxBufferSize()
  if (this->_z.rows() == this->_conv.get_out_channels() &&
      this->_z.cols() == num_frames)
...
nam::wavenet::_LayerArray::_LayerArray(const int input_size,
                                       const int condition_size,
                                       const int head_size, const int channels,
...
void nam::wavenet::_LayerArray::process_(const Eigen::MatrixXf& layer_inputs,
                                         const Eigen::MatrixXf& condition,
  ...
  this->_layer_buffers[0].middleCols(this->_buffer_start, num_frames) =
      _rechannel.GetOutput(num_frames);
...
void nam::wavenet::_LayerArray::set_weights_(
...
nam::wavenet::_Head::_Head(const int input_size, const int num_layers,
  ...
  for (int i = 0; i < num_layers; i++) {
    ...
    dx = channels;
    if (i < num_layers - 1) this->_buffers.push_back(Eigen::MatrixXf());
  }
}

void nam::wavenet::_Head::Reset(const double sampleRate,
...
void nam::wavenet::_Head::process_(Eigen::MatrixXf& inputs,
  ...
  if (num_layers == 1)
    ...
    for (size_t i = 1; i < num_layers; i++) {  // Asserted > 0 layers
...
void nam::wavenet::_Head::set_num_frames_(const long num_frames) {
  for (size_t i = 0; i < this->_buffers.size(); i++) {
    if (this->_buffers[i].rows() == this->_channels &&
        this->_buffers[i].cols() == num_frames)
      continue;  // Already has correct size
    this->_buffers[i].resize(this->_channels, num_frames);
    this->_buffers[i].setZero();  // Shouldn't be needed--these are written to
                                  // before they're used.
  }
}
...
nam::wavenet::WaveNet::WaveNet(
    ...
    : DSP(expected_sample_rate), _head_scale(head_scale) {
  if (with_head) throw std::runtime_error("Head not implemented!");
...
void nam::wavenet::WaveNet::set_weights_(std::vector<float>& weights) {
  ...
  if (it != weights.end()) {
    ...
    for (size_t i = 0; i < weights.size(); i++)
      if (weights[i] == *it) {
        ss << "Weight mismatch: assigned " << i + 1 << " weights, but "
           << weights.size() << " were provided.";
        throw std::runtime_error(ss.str().c_str());
      }
    ss << "Weight mismatch: provided " << weights.size()
...
void nam::wavenet::WaveNet::_set_condition_array(NAM_SAMPLE* input,
                                                 const int num_frames) {
...
void nam::wavenet::WaveNet::process(NAM_SAMPLE* input, NAM_SAMPLE* output,
                                    const int num_frames) {
  ...
  for (size_t i = 0; i < this->_layer_arrays.size(); i++)
    this->_layer_arrays[i].process_(
        ...
        this->_head_arrays[i + 1], num_frames);
  // this->_head.process_(
  //   this->_head_input,
  //   this->_head_output
  //);
...
std::unique_ptr<nam::DSP> nam::wavenet::Factory(
    const nlohmann::json& config, std::vector<float>& weights,
  ...
  for (size_t i = 0; i < config["layers"].size(); i++) {
    ...
    layer_array_params.push_back(nam::wavenet::LayerArrayParams(
        ...
        layer_config["kernel_size"], layer_config["dilations"],
        layer_config["activation"], layer_config["gated"],
