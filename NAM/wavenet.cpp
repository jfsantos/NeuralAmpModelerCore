...
nam::wavenet::_LayerArray::_LayerArray(const int input_size, const int condition_size, const int head_size,
    ...
    : _rechannel(input_size, channels, false), _head_rechannel(channels, head_size, head_bias) {
...
void nam::wavenet::_LayerArray::prepare_for_frames_(const long num_frames) {
  ...
  // -> No illegal writes.
  // -> no rewind needed.
  if (this->_buffer_start + num_frames > this->_get_buffer_size()) this->_rewind_buffers_();
}
...
void nam::wavenet::_LayerArray::set_num_frames_(const long num_frames) {
  ...
  for (size_t i = 0; i < this->_layers.size(); i++) this->_layers[i].set_num_frames_(num_frames);
}

void nam::wavenet::_LayerArray::set_weights_(std::vector<float>::iterator& weights) {
  this->_rechannel.set_weights_(weights);
  for (size_t i = 0; i < this->_layers.size(); i++) this->_layers[i].set_weights_(weights);
...
nam::wavenet::WaveNet::WaveNet(const std::vector<nam::wavenet::LayerArrayParams>& layer_array_params,
  ...
  for (size_t i = 0; i < layer_array_params.size(); i++) {
    ...
    if (i == 0) this->_head_arrays.push_back(Eigen::MatrixXf(layer_array_params[i].channels, 0));
    if (i > 0)
...
void nam::wavenet::WaveNet::set_weights_(std::vector<float>& weights) {
  ...
  for (size_t i = 0; i < this->_layer_arrays.size(); i++) this->_layer_arrays[i].set_weights_(it);
...
void nam::wavenet::WaveNet::SetMaxBufferSize(const int maxBufferSize) {
  ...
  for (size_t i = 0; i < this->_layer_arrays.size(); i++) this->_layer_arrays[i].SetMaxBufferSize(maxBufferSize);
  // this->_head.SetMaxBufferSize(maxBufferSize);
}

void nam::wavenet::WaveNet::_advance_buffers_(const int num_frames) {
  for (size_t i = 0; i < this->_layer_arrays.size(); i++) this->_layer_arrays[i].advance_buffers_(num_frames);
}

void nam::wavenet::WaveNet::_prepare_for_frames_(const long num_frames) {
  for (size_t i = 0; i < this->_layer_arrays.size(); i++) this->_layer_arrays[i].prepare_for_frames_(num_frames);
}
...
std::unique_ptr<nam::DSP> nam::wavenet::Factory(const nlohmann::json& config, std::vector<float>& weights,
  ...
  for (size_t i = 0; i < config["layers"].size(); i++) {
    ...
    layer_array_params.push_back(
        nam::wavenet::LayerArrayParams(layer_config["input_size"], layer_config["condition_size"],
                                       layer_config["head_size"], layer_config["channels"], layer_config["kernel_size"],
                                       layer_config["dilations"], layer_config["activation"], layer_config["gated"],
                                       layer_config["head_bias"]));
  }
  const bool with_head = !config["head"].is_null();
  const float head_scale = config["head_scale"];
  return std::make_unique<nam::wavenet::WaveNet>(layer_array_params, head_scale, with_head, weights,
                                                 expectedSampleRate);
}
