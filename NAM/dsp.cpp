...
long nam::Conv1x1::get_num_weights() const
{
  long num_weights = this->_do_bias ? this->_bias.size() : 0;
  if (this->_is_depthwise)
  {
    num_weights += this->_channels;
  }
  else if (this->_weight.size() > 0)
  {
    const long out_channels = this->_weight.rows();
    const long in_channels = this->_weight.cols();
    num_weights += (out_channels * in_channels) / this->_num_groups;
  }
  return num_weights;
}
...
size_t nam::Conv1x1::copy_weights_to_buffer(float* buffer, size_t buffer_size) const
{
  ...
  if (this->_is_depthwise)
  {
    std::memcpy(buffer + offset, this->_depthwise_weight.data(), this->_depthwise_weight.size() * sizeof(float));
    offset += this->_depthwise_weight.size();
  }
  else if (this->_weight.size() > 0)
  {
    std::memcpy(buffer + offset, this->_weight.data(), this->_weight.size() * sizeof(float));
    offset += this->_weight.size();
  }
