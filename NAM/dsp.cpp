...
void nam::DSP::process(NAM_SAMPLE* input, NAM_SAMPLE* output,
                       const int num_frames) {
  // Default implementation is the null operation
  for (int i = 0; i < num_frames; i++) output[i] = input[i];
}

double nam::DSP::GetLoudness() const {
  if (!HasLoudness()) {
    throw std::runtime_error(
        "Asked for loudness of a model that doesn't know how loud it is!");
  }
  return mLoudness;
}

void nam::DSP::Reset(const double sampleRate, const int maxBufferSize) {
  // Some subclasses might want to throw an exception if the sample rate is
  // "wrong". This could be under a debugging flag potentially.
...
nam::Buffer::Buffer(const int receptive_field,
...
void nam::Buffer::_update_buffers_(NAM_SAMPLE* input, const int num_frames) {
  ...
  // frames needed!
  {
    ...
    if ((long)this->_input_buffer.size() < minimum_input_buffer_size) {
      ...
      while (new_buffer_size < minimum_input_buffer_size) new_buffer_size *= 2;
  ...
  if (this->_input_buffer_offset + num_frames >
      (long)this->_input_buffer.size())
...
nam::Linear::Linear(const int receptive_field, const bool _bias,
...
void nam::Linear::process(NAM_SAMPLE* input, NAM_SAMPLE* output,
                          const int num_frames) {
...
// Factory
std::unique_ptr<nam::DSP> nam::linear::Factory(
    const nlohmann::json& config, std::vector<float>& weights,
    const double expectedSampleRate) {
  const int receptive_field = config["receptive_field"];
  const bool bias = config["bias"];
  return std::make_unique<nam::Linear>(receptive_field, bias, weights,
                                       expectedSampleRate);
}
...
void nam::Conv1D::set_weights_(std::vector<float>::iterator& weights) {
  ...
  for (long i = 0; i < this->_bias.size(); i++) this->_bias(i) = *(weights++);
}

void nam::Conv1D::set_size_(const int in_channels, const int out_channels,
...
void nam::Conv1D::set_size_and_weights_(const int in_channels,
                                        const int out_channels,
                                        const int kernel_size,
                                        const int _dilation, const bool do_bias,
                                        std::vector<float>::iterator& weights) {
  this->set_size_(in_channels, out_channels, kernel_size, do_bias, _dilation);
  this->set_weights_(weights);
}

void nam::Conv1D::process_(const Eigen::MatrixXf& input,
                           Eigen::MatrixXf& output, const long i_start,
                           const long ncols, const long j_start) const {
...
nam::Conv1x1::Conv1x1(const int in_channels, const int out_channels,
                      const bool _bias) {
  this->_weight.resize(out_channels, in_channels);
  this->_do_bias = _bias;
  if (_bias) this->_bias.resize(out_channels);
}
...
void nam::Conv1x1::set_weights_(std::vector<float>::iterator& weights) {
  ...
  if (this->_do_bias)
    for (int i = 0; i < this->_bias.size(); i++) this->_bias(i) = *(weights++);
}

Eigen::MatrixXf nam::Conv1x1::process(const Eigen::MatrixXf& input,
...
void nam::Conv1x1::process_(const Eigen::MatrixXf& input,
                            const int num_frames) {
  assert(num_frames <= _output.cols());
  _output.leftCols(num_frames).noalias() =
      this->_weight * input.leftCols(num_frames);
  if (this->_do_bias) {
