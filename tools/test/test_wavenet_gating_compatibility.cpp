...
class TestWavenetGatingCompatibility {
 public:
  static void test_wavenet_style_gating() {
    ...
    // Verify that the output is the element-wise product of input and gating
    // channels after applying activations
  ...
  static void test_multiple_channels() {
    ...
    // Verify that each input channel is multiplied by corresponding gating
    // channel
    for (int c = 0; c < channels; c++) {
      for (int s = 0; s < num_samples; s++) {
        float input_val = input(c, s);
