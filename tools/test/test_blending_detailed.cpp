...
class TestBlendingDetailed {
 public:
  static void test_blending_with_different_activations() {
    // Test case: 2 input channels, so we need 4 total input channels
    // (2*channels in)
    ...
    // So output should equal the input channels after activation (which is the
    // same as input)
    ...
    // input = input This should be the same as with linear activations
