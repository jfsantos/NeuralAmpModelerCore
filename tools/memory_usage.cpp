...
static constexpr size_t DTCM_WEIGHT_BUFFER_FLOATS = 24576; // 96 KB
...
int main(int argc, char* argv[])
{
  ...
  try
  {
    ...
    printf("\n");
    size_t weight_floats = result.weight_bytes / sizeof(float);

    printf("Weights:  %s bytes (%s) — %s floats\n", format_with_commas(result.weight_bytes).c_str(),
           format_bytes(result.weight_bytes).c_str(), format_with_commas(weight_floats).c_str());
    ...
    printf("\nDTCM:     %s / %s floats (%s / %s) — %s\n",
           format_with_commas(weight_floats).c_str(),
           format_with_commas(DTCM_WEIGHT_BUFFER_FLOATS).c_str(),
           format_bytes(weight_floats * sizeof(float)).c_str(),
           format_bytes(DTCM_WEIGHT_BUFFER_FLOATS * sizeof(float)).c_str(),
           weight_floats <= DTCM_WEIGHT_BUFFER_FLOATS ? "FITS" : "OVER — will use heap");

    if (json_weight_count > 0)
    {
      printf("\nJSON weights: %zu values (%s bytes)\n", json_weight_count,
             format_with_commas(json_weight_count * sizeof(float)).c_str());
      if (json_weight_count != weight_floats)
        printf("  (storage > JSON count due to grouped convolution padding)\n");
    }
  }
  catch (const std::exception& e)
  {
